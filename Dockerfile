# cdar-client-kit — standalone Rung 1/2 conformance testing kit.
#
...
# default).
#
# cdar-contracts is vendored as a pre-built wheel in vendor/ (15KB,
# cdar_contracts-1.0.0-py3-none-any.whl) so the build needs no cross-repo
# auth, no BuildKit named contexts, and no internet access beyond PyPI for
# fastapi/uvicorn/jinja2. When cdar-contracts bumps a version, rebuild the
...
# Install the vendored cdar-contracts wheel, then the kit itself (editable,
# so web/templates/ and stage1/templates/ resolve from /app at runtime).
RUN pip install --no-cache-dir vendor/cdar_contracts-*.whl
...
#   docker run -v $(pwd)/data:/app/data cdar-client-kit \
