"""
PHI redaction engine for Stage 2 (anonymized extract, Path B only).

Vendored from IndicinaAdmin/HealthClawGuardrails `scripts/healthclaw_redact.py`
(2026-06-16) — the standalone, zero-dependency mirror of that project's
guardrail-proxy redaction rules, built for exactly this "redact FHIR
resources before anything touches disk" use case. Uses SHA-256 hashing for
identifiers (not last-4-chars, which is what the live-proxy variant in that
repo's `r6/redaction.py` uses) so cross-resource joins on a hashed identifier
still work for Level 3-5 measure checks while remaining irreversible.

The proxy-relay mode from the upstream script is intentionally dropped here —
Stage 2 runs entirely offline against local files; nothing should call out
over HTTP with PHI-shaped payloads, even optionally.

If this drifts from the upstream HealthClaw version, re-sync manually; there
is no shared package today (see specs/dqar-06 Stage 2 section / open items).
...
Import: from stage2.redact import redact, RedactionStats
"""
...
# Keys that indicate PHI in non-FHIR shaped payloads (flattened dashboards,
# convenience responses, etc). Matching is case-insensitive on the exact key
# name, not substring, to avoid false positives on "patient_name_id" or
# similar identifier-shaped keys.
...
def _hash_identifier(value: str, salt: str = "") -> str:
    """SHA-256 prefix of the identifier value. Deterministic when salt is fixed."""
